...
export default function EditorPage() {
  ...
  const handleSave = async () => {
    ...
    // 2. 저장 후 자동 일관성 검사 (백그라운드)
    if (!content.trim()) return;
    setChecking(true);
    try {
      const cons = await checkConsistency(content.trim(), currentProject.settingData || null);
      setConsistencyReport(cons);
      // 컨텍스트 + DB에 저장
      updateProject(currentProject.id, { consistencyReport: cons });
    } catch (err) {
      console.warn('자동 일관성 검사 실패:', err);
    } finally {
      setChecking(false);
    }
  };
  ...
  const selectedEpTitle = selectedEpisodeId
    ...
    <div className="h-full flex flex-col">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 text-xs mt-0.5">
              ...
              {selectedEpTitle
                ? ` › ${selectedEpTitle.chapterNumber ?? ''}화 ${selectedEpTitle.title}`
                : ' › 세계관 전체'}
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {checking && (
              <span className="text-xs text-blue-500 dark:text-blue-400 animate-pulse">🔍 일관성 검사 중...</span>
            )}
            {!isSaved && !checking && (
            ...
            <button
              onClick={handleSave}
              disabled={checking}
              className={`py-1.5 px-4 rounded-lg font-semibold text-sm transition-colors disabled:opacity-60 ${
                isSaved && !checking
      ...
      <div className="flex-1 flex overflow-hidden">
            ...
            {showNewEpisode ? (
              <div className="space-y-1.5">
                ...
                <div className="flex gap-1">
                  <button onClick={handleAddEpisode} className="flex-1 text-xs py-1 bg-blue-600 hover:bg-blue-700 text-white rounded transition-colors">추가</button>
                  <button onClick={() => { setShowNewEpisode(false); setNewEpisodeTitle(''); }} className="flex-1 text-xs py-1 bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded transition-colors">취소</button>
                </div>
              </div>
            ) : (
              <button
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {/* 일관성 검사 결과 (저장 후 자동 표시) */}
          {consistencyReport && (
            <div className="flex-shrink-0 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-y-auto max-h-72">
              <div className="px-5 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between sticky top-0 bg-white dark:bg-gray-800 z-10">
                <p className="text-sm font-bold text-gray-800 dark:text-white">🔍 일관성 검사 결과</p>
                <button
              ...
              <div className="px-5 py-4">
                <ConsistencyReport report={consistencyReport} />
              </div>
            </div>
          )}
        </div>
